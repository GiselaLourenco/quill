  ...
  return (
    <div className="flex h-full flex-col bg-paper">
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-5 py-6">
        {passo === 1 && <Passo1 nome={nome} onNome={setNome} duracao={duracao} onDuracao={setDuracao} />}
...
function Passo1({ nome, onNome, duracao, onDuracao }: { nome: string; onNome: (v: string) => void; duracao: Duracao; onDuracao: (v: Duracao) => void }) {
  return (
    <>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-3 gap-3">
          {DURACOES.map((d) => {
            ...
            return (
              <button key={d.id} type="button" onClick={() => onDuracao(d.id)} className={`flex flex-col items-center border-2 border-ink px-2 py-3 font-display text-xs uppercase tracking-wider ${sel ? "translate-x-[2px] translate-y-[2px] bg-mustard text-ink shadow-none" : "bg-paper text-ink shadow-hard-sm"}`}>
